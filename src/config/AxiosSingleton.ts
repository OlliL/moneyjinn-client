...
export class AxiosSingleton {
  ...
  public static getInstance(): AxiosSingleton {
    if (!AxiosSingleton.instance) {
      ...
      AxiosSingleton.instance.axiosInstance = axios.create({
        baseURL: AxiosSingleton.instance.getWebRoot(),
      ...
      applyAuthTokenInterceptor(AxiosSingleton.instance.axiosInstance, {
        requestRefresh,
      });

      AxiosSingleton.instance.axiosInstance.interceptors.response.use(
        (response) => {
          return response;
        },
        (error) => {
