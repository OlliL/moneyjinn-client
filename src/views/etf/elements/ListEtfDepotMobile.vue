<template>
  <div data-testid="etf-depot-mobile-accordion" class="md:hidden w-full max-w-md mx-auto px-2 mb-6 space-y-3">
    <Accordion type="multiple" class="space-y-2">
      ...
      >
        <AccordionTrigger class="hover:no-underline">
          ...
          >
            <div class="flex flex-col items-start text-left min-w-0">
              <span class="font-bold text-sm truncate">{{ timestampString(flow) }}</span>
              <span :class="amountClass(flow)" class="text-xs">{{ amountString(flow) }}</span>
            </div>
        ...
        <AccordionContent :data-testid="`etf-depot-mobile-content-${flow.etfflowid}`" class="pt-2">
          <div class="grid grid-cols-1 gap-2 text-sm bg-muted/40 p-3 rounded-md">
            <div class="flex justify-between gap-2">
              <span class="text-muted-foreground">{{ $t("ETFFlow.price") }}</span>
              <SpanAmount :amount="flow.price" :decimal-places="3" />
            </div>
            <div class="flex justify-between gap-2">
              <span class="text-muted-foreground">{{ $t("ETFFlow.sumprice") }}</span>
              <SpanAmount :amount="flow.amount * flow.price" />
            </div>
            <div v-if="showLumpSum" class="flex justify-between gap-2">
              <span class="text-muted-foreground">{{ $t("ETFFlow.preliminaryLumpSum") }}</span>
              <SpanAmount
